...
import MixamoModelPicker from "../MixamoModelPicker";
...
class CameraViewer extends React.Component {
    ...
    componentDidMount=async ()=>{
        try {
            ...
            this.posenet3dModel = await posenet3d.load();
        }
        catch (e) {
    ...
    /**
     * grab the current frame and pass it into the facemesh posenet3d
     */
    makePredictions=async()=>{
        try {
            ...
            // const poses2d = await this.posenetModel.estimatePoses(inputFrame, {
            //     decodingMethod: 'single-person',
            //     maxDetections: 1,
            //     scoreThreshold: minPartConfidence,
            //     nmsRadius: nmsRadius
            // });
            const pose3D = await  this.posenet3dModel.estimatePoses(inputFrame);
            // clear canvas
            ...
            this.drawCanvasCtx.translate(0, 0);
            if(pose3D) {
                let points = pose3D.key;
                points.forEach(pt => {
                    let {x, y, score} = pt;
                    x = Math.ceil(x * videoWidth);
                    y = Math.ceil(y * videoHeight);
                    if (score > 0.2) {
                        this.drawCanvasCtx.beginPath();
                        this.drawCanvasCtx.arc(x, y, 2, 0, 2 * Math.PI);
            ...
            //         this.drawCanvasCtx.save();
            //         this.drawCanvasCtx.translate(0, 0);
            //         VisUtil.drawPose(this.drawCanvasCtx, poses2d[0],
            //             minPoseConfidence, minPartConfidence, 1, "green");
            //         this.updatePosenetKeypoints(poses2d[0]);
            //     }
            // }
            // else {
            ...
            <Draggable >
                <Card elevation={20} style={{ zIndex: 1, position: "absolute", top: 50, left: 50}}>
                    ...
                    <Grid container
                        ...
                        <Grid item xs={12}>
                            <DevicePicker ref={this.deviceSelectRef}/>
                            {/*<MixamoModelPicker></MixamoModelPicker>*/}
                        </Grid>
                        <Grid item xs={12}>
